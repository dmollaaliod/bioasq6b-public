...
class BaseRegression:
    ...
    def __init__(self, corpusFile, rougeFile, add_snippets=False,
        ...
        self.rouge = dict()
        with codecs.open(rougeFile) as f:
            reader = csv.reader(f)
            header = next(reader)
    ...
    def save_for_hpc(self, savepath, corpusfile, rougefile, n_folds=10, small_data=False):
        ...
        fold = 0
        for traini, testi in KFold(len(indices), n_folds):
            fold += 1
    ...
    def run_for_hpc(self, loadpath=None, n_folds=10, fold=0, resultspath="", small_data=False):
        "Run models"
        def iterator_kfold(datalength, n_folds):
            "Iterator wrapper for KFold"
            for traini, testi in KFold(datalength, n_folds):
                yield traini, testi
        ...
        if loadpath is None:
            ...
            random.seed(1234)
            random.shuffle(indices)
            kfolds = iterator_kfold(len(indices), n_folds)
...
""")
        ...
        json_summaries_file = "%s/crossvalidation/crossvalidation_%i_summaries.json" % (resultspath, fold)
        ...
        json_gold_file = "%s/crossvalidation/crossvalidation_%i_gold.json" % (resultspath, fold)
        ...
        #bioasq_output_file = 'crossvalidation/bioasq_3b_05_%i.json' % fold
